...
export function calculateProgressScore(uomType, target, actual) {
  ...
  // Handle division by zero or invalid numbers (for numeric types)
  ...
  switch (uomType) {
    case 'numeric_max':
      // Higher actual = better (e.g. Sales). Formula: (actual / target) * 100
      score = targetVal > 0 ? (actualVal / targetVal) * 100 : 0;
      break;

    case 'numeric_min':
      // Lower actual = better (e.g. Cost). Formula: (target / actual) * 100
